...
def conv_cond_concat(x, y):
    ...
    x_shapes = tf.shape(x)
    y_shapes = tf.shape(y)
    ones_tensor = tf.ones([x_shapes[0], x_shapes[1], x_shapes[2], y_shapes[3]])
    return concat([x, y * ones_tensor], 3)
...
def conv2d(
    ...
    with tf.variable_scope(name):
        ...
        w = tf.get_variable(
            name='w',
            shape=[k_h, k_w, input_.get_shape()[-1], output_dim],
...
def resizeconv(
    ...
    with tf.variable_scope(name):
        ...
        w = tf.get_variable(
            name='w',
            shape=[k_h, k_w, resized.get_shape()[-1], output_shape[-1]],
        	initializer=tf.truncated_normal_initializer(stddev=stddev)
        )
...
def deconv2d(
    ...
    with tf.variable_scope(name):
        ...
        w = tf.get_variable(
            name='w',
            shape=[k_h, k_w, output_shape[-1], input_.get_shape()[-1]],
            initializer=tf.random_normal_initializer(stddev=stddev)
        )
         
        deconv = tf.nn.conv2d_transpose(input_, w, output_shape=out_shape, strides=[1, d_h, d_w, 1])
        biases = tf.get_variable('biases', [output_shape[-1]], initializer=tf.constant_initializer(0.0))
